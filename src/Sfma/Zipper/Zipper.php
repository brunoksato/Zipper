...
namespace Sfma\Zipper;

use Sfma\Zipper\Repositories\RepositoryInterface;
use Exception;
...
class Zipper
{
    ...
    public function make($pathToFile, $type = 'zip')
    {
        ...
        $objectOrName = $type;
        if (is_string($type)) {
            $objectOrName = 'Sfma\Zipper\Repositories\\' . ucwords($type) . 'Repository';
        }

        if (!is_subclass_of($objectOrName, 'Sfma\Zipper\Repositories\RepositoryInterface')) {
    ...
    private function extractFilesInternal($path, callable $matchingMethod)
    {
        ...
        $this->repository->each(function ($fileName) use ($path, $matchingMethod, $self) {
            ...
            if (!empty($currentPath) && !Str::starts_with($fileName, $currentPath)) {
                return;
            }
